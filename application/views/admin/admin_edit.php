				...
				<div class="col-md-4 col-md-offset-4">
					...
					<form action="" method="POST">
						<div class="form-group <?=form_error('id')? 'has-error' : null?>">
							<label>ID *</label>							
							<input readonly="" type="text" name="id" class="form-control" value="<?=$this->input->post('id')?? $row->admin_id?>">
						</div>
						...
						<div class="form-group <?=form_error('phone')? 'has-error' : null?>">
							<label>Phone *</label>
							<input autocomplete="off" type="text" name="phone" class="form-control" value="<?=$this->input->post('phone')?? $row->phone?>">
							<?=form_error('phone')?>
						</div>
